...
* **Resource UI Module Generation**: Read [resource_ui_module_developer.md](file:///f:/LITTLE%20LEAP/AQL/References/Prompt%20Library/Initialization/resource_ui_module_developer.md) (machine-optimized, step-by-step: classify maturity tier, scaffold the `src/_resource/` domain layer FIRST, scaffold the injection-relay UI Composable, then build Index/Add/Edit/View per the strict three-layer import boundary — backed by canonical spec [RESOURCE_UI_MODULE_DEVELOPER_GUIDE.md](file:///f:/LITTLE%20LEAP/AQL/Documents/RESOURCE_UI_MODULE_DEVELOPER_GUIDE.md)). Always load this first for any request to build/generate/extend a resource's UI — its own Scope Boundary hands off to a narrower prompt below if the task turns out to be single-piece.
...
This project is indexed by GitNexus as **little-leap-aql** (14060 symbols, 22742 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
