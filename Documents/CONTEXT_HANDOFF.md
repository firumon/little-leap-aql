...
- **Supplier Quotation Flow (2026-04-25)**: `/operations/quotations` now resolves to custom pages under `FRONTENT/src/pages/Operations/Quotations/` and composables under `operations/supplierQuotations/`. First save captures `QUOTED`, `PARTIAL`, or `DECLINED` responses into `SupplierQuotations` / `SupplierQuotationItems`, marks the matching `RFQSuppliers` row `RESPONDED`, and advances `Procurements` from `RFQ_SENT_TO_SUPPLIERS` to `QUOTATIONS_RECEIVED` without implementing comparison/scoring or PO generation.
