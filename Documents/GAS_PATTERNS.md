...
- **Single-Gate Verification & Resync**: `verifySessionProof(token, sessionKey)` runs at the dispatcher entrance in `doPost`. It validates that the generation is within an asymmetric window (`stored - 2 <= clientGen <= stored + 30`; `SESSION_GEN_WINDOW_BACK = 2`, `SESSION_GEN_WINDOW_AHEAD = 30`) to tolerate network drops and concurrent requests. When an in-flight request is valid but out-of-window, the rejection returns an encoded resync token (`sessionResync`) derived using the same session cryptography so the client can resync its counter and retry once without logging out. The bare generation is never revealed.
