...
Custom components for Operations resources must be placed in specific directories to be discovered by the 12-tier resolver.
...
*Note: For the Operations module, `{ScopeFolder}` is always `Operations`.*
...
| **List** | `Records` | Card list container with loading/empty handlers | `components/_common/Operations/List/Records.vue` |
...
| **View** | `ActionBar` | Custom outcomes & edit triggers | `components/_common/Operations/View/ActionBar.vue` |
| **View** | `Details` | Record values grid | `components/_common/Operations/View/Details.vue` |
| **View** | `Parent` | Displays parent resource record summary card | `components/_common/View/Parent.vue` |
| **View** | `Children` | Loop of child resource lists | `components/_common/Operations/View/Children.vue` |
...
| **Add** | `Form` | Composite data input fields | `components/_common/Operations/Add/Form.vue` |
| **Add** | `Children` | Sub-tables for child record rows | `components/_common/Operations/Add/Children.vue` |
...
### Goal: Customize the view details component for "Purchase Requisitions" for tenant `C456`

1. **Set `customUIName`**: In the resource configuration for `purchase-requisition`, ensure `ui.customUIName` is set to `"C456"`.
2. **Create the file**:
   - The resource slug is `purchase-requisition` (PascalCase: `PurchaseRequisition`).
   - The page is `View`.
   - The component to override is `Details`.
   - Create the file path:
     `src/components/_custom/C456/Operations/PurchaseRequisition/View/Details.vue`
