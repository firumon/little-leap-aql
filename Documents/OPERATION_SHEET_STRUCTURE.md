...
- Outlet restock allocation is stored on `OutletRestockItems` rows; `OutletDeliveryItems` links delivery headers to allocated ORSI rows. Warehouse stock moves at allocation time, and delivery item completion posts outlet stock.
...
| `OutletDeliveries` | Multi-outlet delivery header. | `Date`, `UserName`, `Progress`, `Status` | `Progress = DRAFT`, `Status = Active`; progress is derived from linked `OutletDeliveryItems` and moves to `IN_TRANSIT`, `COMPLETED`, or `CANCELLED`. |
| `OutletDeliveryItems` | Delivery child rows linking one OD to one ORSI. | `OutletDeliveryCode`, `OutletRestockItemCode`, `Progress`, `Status` | `Progress = IN_TRANSIT`; item completion changes to `DELIVERED` and stamps delivery audit fields; composite uniqueness is `OutletDeliveryCode + OutletRestockItemCode`. |
...
- `OutletDeliveries`: `Code`, `Date`, `UserName`, `Progress`, in-transit/completed progress stamp/comment columns, cancel audit fields, `Status`, `AccessRegion`, audit columns.
- `OutletDeliveryItems`: `Code`, `OutletDeliveryCode`, `OutletRestockItemCode`, `Progress`, delivered progress stamp/comment columns, `Status`, `AccessRegion`, audit columns.
