...
- **First Save Workflow**: The first saved quotation response marks the matching `RFQSuppliers` row as `RESPONDED`. If the matching row is still `ASSIGNED`, it stamps blank `SentDate` with today. It advances the linked `Procurements` row from `RFQ_SENT_TO_SUPPLIERS` to `QUOTATIONS_RECEIVED`. Later quotation edits do not repeat those workflow updates.
