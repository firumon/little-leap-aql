...
-Menu groups are generated by flattening every `menu` entry in `resources[].ui.menus` from the authorized login payload (grouped by `menu.group` and sorted by `menu.order`).
-Only menu entries where `menu.show !== false` and `menuAccess` (if present) passes the current user's permissions are shown; when `menuAccess` is absent, the fallback is `permissions.canRead === true`.
