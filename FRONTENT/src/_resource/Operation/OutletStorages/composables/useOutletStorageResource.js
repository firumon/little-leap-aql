 ...
 *
 * (OutletCode, SKU) is the key. A second row for the same pair is a stale local copy, not
 * extra stock, so the index keeps the last one and drops the rest — see `indexOutletStock`.
 *
 * ── NON-DESTRUCTIVE ──
 ...
 *
 * (OutletCode, SKU) is the sheet's key: one balance row per SKU per outlet. The client can
 * still hold two, because a row GAS deleted when the balance hit zero is never removed from
 * the local cache, and the later restock of that SKU is written back under a NEW Code. Both
 * survive here and the SKU shows up twice. The last row wins — it is the live one.
 */
