<template>
  <div :class="gutterClass">
    <!-- DRAFT: the request has never been submitted, so the only question is
         whether this save submits it. There is nothing to comment on yet. -->
    <q-card v-if="isDraftState" class="aql-premium-card bg-primary-light" flat>
      <q-card-section>
    ...
    <!-- REVISION_REQUIRED: the request is already in the workflow and this save
         ...
         Going back to draft is not offered — the request has left that state. -->
 ...
 * one control, not both greyed against each other:
 *
 *   DRAFT              → "is this still a draft?"   (the toggle)
 *   REVISION_REQUIRED  → "what changed?"            (the comment)
 *
 ...
 * handler does not perform. A draft has never been submitted, so a submission
 * comment has no reader yet.
 *
