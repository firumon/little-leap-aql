...
Purpose: track tenant-specific section overrides under `components/operation/_custom/`.
...
```
components/operation/_custom/{CustomUIName}/{Entity}{Section}.vue
```

If no custom section is found, resolution falls through to entity-custom (`components/operation/{Entity}/{Section}.vue`) then to the default (`components/operation/_common/Operation{Action}{Section}.vue`).
...
| Scenario | Use |
|----------|-----|
| Only 1-2 sections need changes (e.g. different header layout) | Section override here (`components/operation/_custom/`) |
| Most/all sections need changes or the page layout itself changes | Full page override in `pages/operation/_custom/` |
