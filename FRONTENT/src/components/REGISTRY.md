...
| `MasterRecordCard` | Individual master record card with click-to-open detail (no status badge). | `{ row: Object, resolvePrimaryText: Function, resolveSecondaryText: Function }` | `open-detail(row)` | `FRONTENT/src/components/Masters/MasterRecordCard.vue` |
