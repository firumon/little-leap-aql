...
|---|---|---|---|---|
| `MasterHeader` | Master page header with title/subtitle, sync status, record counters, and toolbar-level report buttons. | `{ config: Object, filteredCount: Number, totalCount: Number, loading: Boolean, backgroundSyncing: Boolean, reports: Array, isGenerating: Boolean }` | `reload()`, `generate-report(report)` | `FRONTENT/src/components/Masters/MasterHeader.vue` |
| `MasterToolbar` | Search input for master list filtering. | `{ searchTerm: String }` | `update:searchTerm(value)` | `FRONTENT/src/components/Masters/MasterToolbar.vue` |
...
| `MasterBreadcrumb` | Breadcrumb navigation for resource sub-route pages (Resource > Code > Action). | `{ scope: String, resourceSlug: String, config: Object, code: String, action: String }` | — | `FRONTENT/src/components/Masters/MasterBreadcrumb.vue` |
