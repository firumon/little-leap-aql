...
| `useListSectionResolver` | Resolves list-page section components with custom entity override fallback (`pages/Masters/{Entity}/List{Section}.vue`) and default components from `components/Masters/`. | `(resourceSlug: Ref<String>)` | `{ HeaderComponent, ReportBarComponent, ToolbarComponent, RecordsComponent, sectionsReady }` | `FRONTENT/src/composables/useListSectionResolver.js` |
