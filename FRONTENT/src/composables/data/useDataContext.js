...
export function useDataContext () {
  ...
  const topN = (map, n, label) => [...(map?.entries ? map.entries() : [])]
    .map(([key, value]) => ({ label: label ? label(key) : key, value }))
    .sort((a, b) => b.value - a.value)
    .slice(0, n)

  const mean = (numbers) => (numbers && numbers.length)
    ? numbers.reduce((a, b) => a + b, 0) / numbers.length
    : null

  const daysSince = (value) => {
    const t = ms(value)
    return t === null ? null : (Date.now() - t) / DAY
  }
  ...
  const countAt = (list, range, predicate, dateField = 'Date') =>
    (list || []).filter((r) => inRange(r[dateField], range) && (!predicate || predicate(r))).length

  const progressOptions = (list) =>
    [...new Set((list || []).map((r) => r.Progress).filter(Boolean))]
