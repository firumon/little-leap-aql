...
export function useOutletVisits() {
  ...
  async function planVisit(outletCode, date, comment = '') { if (!allowed('create')) { $q.notify({ type: 'negative', message: 'You do not have permission to plan a visit.', position: 'top' }); return false } if (!text(outletCode) || !text(date)) { $q.notify({ type: 'warning', message: 'Outlet and date are required.', position: 'top' }); return false } saving.value = true; try { const result = await resourceIoStore.createResourceRecord('OutletVisits', { OutletCode: text(outletCode), Date: text(date), Progress: 'PLANNED', ProgressPlannedComment: text(comment), Status: 'Active' }); if (!result?.success) { $q.notify({ type: 'negative', message: result?.error || result?.message || 'Failed to create visit.', position: 'top' }); return false } $q.notify({ type: 'positive', message: 'Visit planned.', position: 'top' }); return true } finally { saving.value = false } }
  ...
  async function reloadIndex(forceSync = false) { loading.value = true; try { await resourceIoStore.fetchResources(['OutletVisits', 'Outlets'], { forceSync }) } finally { loading.value = false } }
  ...
  async function createVisit() { if (!allowed('create')) { $q.notify({ type: 'negative', message: 'You do not have permission to create a visit.', position: 'top' }); return false } if (!text(form.value.OutletCode) || !text(form.value.Date)) return $q.notify({ type: 'warning', message: 'Outlet and date are required.', position: 'top' }); saving.value = true; try { const result = await resourceIoStore.createResourceRecord('OutletVisits', { OutletCode: text(form.value.OutletCode), Date: text(form.value.Date), Progress: 'PLANNED', ProgressPlannedComment: text(form.value.ProgressPlannedComment), Status: 'Active' }); if (!result?.success) return $q.notify({ type: 'negative', message: result?.error || result?.message || 'Failed to create visit.', position: 'top' }); $q.notify({ type: 'positive', message: 'Visit planned.', position: 'top' }); nav.goTo('index') } finally { saving.value = false } }
  async function completeVisit(visit, fields = {}) { if (!allowed('update')) { $q.notify({ type: 'negative', message: 'You do not have permission to complete a visit.', position: 'top' }); return false } if (visitProgress(visit) !== 'PLANNED') { $q.notify({ type: 'warning', message: 'Only planned visits can be completed.', position: 'top' }); return false } saving.value = true; try { const nextDate = addDaysISO(visit.Date, visitFrequencyDays(visit.OutletCode)); const result = await resourceIoStore.runBatchRequests([executeActionRequest('OutletVisits', visit.Code, OUTLET_ACTIONS.completeVisit, { ProgressCompletedComment: text(fields.ProgressCompletedComment || fields.StatusCompletedComment || fields.StatusComment || fields.Comment) }), plannedVisitRequest(visit, nextDate, 'Auto planned after completed visit')]); if (responseFailed(result)) { $q.notify({ type: 'negative', message: failureMessage(result, 'Failed to complete visit.'), position: 'top' }); return false } $q.notify({ type: 'positive', message: 'Visit completed and next visit planned.', position: 'top' }); return true } finally { saving.value = false } }
  async function postponeVisit(visit, fields = {}) { if (!allowed('update')) { $q.notify({ type: 'negative', message: 'You do not have permission to postpone a visit.', position: 'top' }); return false } if (!text(fields.ProgressPostponedComment || fields.StatusPostponedComment || fields.StatusComment || fields.Comment) || !text(fields.Date)) { $q.notify({ type: 'warning', message: 'Reason and new date are required.', position: 'top' }); return false } saving.value = true; try { const create = { OutletCode: visit.OutletCode, Date: text(fields.Date), Progress: 'PLANNED', ProgressPlannedComment: '', Status: 'Active' }; const result = await resourceIoStore.runBatchRequests([executeActionRequest('OutletVisits', visit.Code, OUTLET_ACTIONS.postponeVisit, { ProgressPostponedComment: text(fields.ProgressPostponedComment || fields.StatusPostponedComment || fields.StatusComment || fields.Comment) }), resourceCreateRequest('OutletVisits', create)]); if (responseFailed(result)) { $q.notify({ type: 'negative', message: failureMessage(result, 'Failed to postpone visit.'), position: 'top' }); return false } $q.notify({ type: 'positive', message: 'Visit postponed.', position: 'top' }); return true } finally { saving.value = false } }
  async function cancelVisit(visit, fields = {}) { if (!allowed('update')) { $q.notify({ type: 'negative', message: 'You do not have permission to cancel a visit.', position: 'top' }); return false } const reason = text(fields.ProgressCancelledComment || fields.StatusCancelledComment || fields.StatusComment || fields.Comment || fields.reason || fields); if (!reason) { $q.notify({ type: 'warning', message: 'Cancellation reason is required.', position: 'top' }); return false } saving.value = true; try { const requests = [executeActionRequest('OutletVisits', visit.Code, OUTLET_ACTIONS.cancelVisit, { ProgressCancelledComment: reason })]; if (text(fields.Date || fields.nextDate)) requests.push(plannedVisitRequest(visit, text(fields.Date || fields.nextDate), 'Auto planned after cancelled visit')); const result = requests.length === 1 ? await resourceIoStore.executeResourceAction('OutletVisits', visit.Code, OUTLET_ACTIONS.cancelVisit, { ProgressCancelledComment: reason }) : await resourceIoStore.runBatchRequests(requests); if (requests.length === 1 ? !result?.success : responseFailed(result)) { $q.notify({ type: 'negative', message: requests.length === 1 ? (result?.error || result?.message || 'Failed to cancel visit.') : failureMessage(result, 'Failed to cancel visit.'), position: 'top' }); return false } $q.notify({ type: 'positive', message: text(fields.Date || fields.nextDate) ? 'Visit cancelled and next visit planned.' : 'Visit cancelled.', position: 'top' }); return true } finally { saving.value = false } }
  function getVisit(code) { return visits.items.value.find(row => row.Code === code) || null } function outletLabel(code) { const row = outlets.items.value.find(item => item.Code === code); return row ? text(row.Name) || code : code } function navigateTo(code) { nav.goTo('view', { code }) } function navigateToAdd() { nav.goTo('add') } function cancel() { nav.goTo('index') }
