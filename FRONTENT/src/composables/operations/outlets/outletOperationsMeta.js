...
const META = {
  ...
  DRAFT: ['Draft', 'grey'], PENDING_APPROVAL: ['Pending Approval', 'orange'], REVISION_REQUIRED: ['Revision Required', 'warning'], APPROVED: ['Approved', 'positive'], PARTIALLY_DELIVERED: ['Partially Delivered', 'info'], DELIVERED: ['Delivered', 'positive'], REJECTED: ['Rejected', 'negative'], SCHEDULED: ['Scheduled', 'warning'],
  PENDING_INVOICE_GENERATION: ['Pending Invoice', 'warning'], INVOICE_GENERATED: ['Invoice Generated', 'positive']
}
...
export function sortTime(row = {}) { const v = row.UpdatedAt || row.CreatedAt || row.ScheduledAt || row.DeliveredAt || row.Date || ''; return typeof v === 'number' ? v : (Date.parse(text(v)) || 0) }
