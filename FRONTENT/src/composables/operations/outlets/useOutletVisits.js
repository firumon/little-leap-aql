...
export function useOutletVisits() {
  const $q = useQuasar(); const resourceIoStore = useResourceIoStore(); const nav = useResourceNav()
  ...
  async function planVisit(outletCode, date, comment = '') { if (!text(outletCode) || !text(date)) { $q.notify({ type: 'warning', message: 'Outlet and date are required.', position: 'top' }); return false } saving.value = true; try { const result = await resourceIoStore.createResourceRecord('OutletVisits', { OutletCode: text(outletCode), Date: text(date), Progress: 'PLANNED', ProgressPlannedComment: text(comment), Status: 'Active' }); if (!result?.success) { $q.notify({ type: 'negative', message: result?.error || result?.message || 'Failed to create visit.', position: 'top' }); return false } $q.notify({ type: 'positive', message: 'Visit planned.', position: 'top' }); return true } finally { saving.value = false } }
