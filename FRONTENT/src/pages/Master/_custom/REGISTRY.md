...
Purpose: track tenant-specific full-page overrides under `pages/master/_custom/`.
...
```
pages/master/_custom/{CustomUIName}/{Entity}.vue          → Index page
pages/master/_custom/{CustomUIName}/{Entity}{Action}.vue  → View/Add/Edit/Action pages
```

If no custom page is found, resolution falls through to entity-custom (`pages/master/{Entity}/`) then to the default (`pages/master/_common/`).
...
| Scenario | Use |
|----------|-----|
| Page layout/structure itself needs to change (e.g. different section order, extra panels) | Full page override here (`pages/master/_custom/`) |
| Only 1-2 sections need changes (e.g. different header layout) | Section override in `components/master/_custom/` instead |
