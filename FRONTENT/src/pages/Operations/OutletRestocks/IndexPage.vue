<template>
  <q-page padding>
        ...
        <q-list separator>
          <q-item v-for="row in group.items" :key="row.Code" clickable @click="navigateTo(row.Code)">
            <q-item-section>
              ...
              <q-item-label caption>{{ row.OutletCode }} - {{ row.Date }} - {{ row.RequestedUser }}</q-item-label>
            </q-item-section>
            <q-item-section side>
    ...
    <div v-else class="text-center q-pa-xl text-grey">No outlet restocks found.</div>
...
const { loading, searchTerm, groups, reloadIndex, navigateTo, navigateToAdd } = flow
