<template>
  <q-page padding>
    <div class="row items-center q-mb-md">
      ...
      <q-space />
      <q-input v-model="searchTerm" dense outlined placeholder="Search" />
        ...
        <q-list separator>
          <q-item v-for="row in group.items" :key="row.Code" clickable @click="navigateTo(row.Code)">
            <q-item-section>
              ...
              <q-item-label caption>{{ outletLabel(row.OutletCode) }} - {{ row.Date }}</q-item-label>
            </q-item-section>
            <q-item-section side>
              <OutletProgressChip :progress="visitProgress(row)" />
            </q-item-section>
          </q-item>
        </q-list>
      </q-expansion-item>
    ...
    <div v-else class="text-center q-pa-xl text-grey">No outlet visits found.</div>
...
import { visitProgress } from '../../../composables/operations/outlets/outletOperationsMeta.js'
...
const { loading, searchTerm, groups, reloadIndex, navigateTo, navigateToAdd, outletLabel } = flow
