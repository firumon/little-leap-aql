  ...
  <div class="rfq-index">
    ...
    <template v-else>
      <div v-for="group in visibleGroups" :key="group.key">
        <div class="rfq-group row items-center q-pa-sm cursor-pointer" :style="{ borderLeftColor: group.color }" @click="toggleGroup(group.key)">
        ...
        <div v-if="isGroupExpanded(group.key)" class="rfq-records">
              ...
              <div class="row items-start no-wrap">
                <div class="col">
                  <div class="row items-center q-gutter-xs">
                    ...
                    <q-chip dense outline>{{ row.Progress || 'DRAFT' }}</q-chip>
                  </div>
                  <div class="text-subtitle2 q-mt-xs">{{ row.PurchaseRequisitionCode || '-' }} · {{ row.ProcurementCode || '-' }}</div>
                  <div class="text-caption text-grey-7">
                    RFQ {{ formatDate(row.RFQDate) }} · Deadline
                    <span :class="isOverdue(row.SubmissionDeadline) ? 'text-negative text-weight-bold' : ''">
                      {{ formatDate(row.SubmissionDeadline) }}
                    </span>
                  </div>
                  <div class="q-mt-xs row q-gutter-xs">
                    <q-chip dense icon="local_shipping">{{ row.ShippingTermMode || 'ANY' }} {{ row.ShippingTerm || '' }}</q-chip>
                    <q-chip dense icon="payments">{{ row.PaymentTermMode || 'ANY' }} {{ row.PaymentTerm || '' }}</q-chip>
                    <q-chip dense icon="call_split">Split {{ row.AllowSplitShipment || 'No' }}</q-chip>
                    <q-chip dense icon="inventory">Partial {{ row.AllowPartialDelivery || 'No' }}</q-chip>
                  </div>
                </div>
      ...
      <div v-if="!totalVisible" class="text-center q-py-xl text-grey-6">
        ...
        <div class="q-mt-sm">No RFQs found</div>
      </div>
    </template>

    <q-page-sticky position="bottom-right" :offset="[16, 22]" style="z-index:30">
      <q-btn v-if="permissions.canWrite" round unelevated icon="add" color="primary" class="rfq-fab" @click="navigateToAdd">
        <q-tooltip>Create RFQ</q-tooltip>
      </q-btn>
    </q-page-sticky>
  </div>
</template>
...
<style scoped>
.rfq-index { display: grid; gap: 8px; }
.rfq-group {
