<template>
  <q-page class="aql-page-container" :class="pageProps.pageClass">
    ...
    <!-- Floating page action / FAB — mounted OUTSIDE the animated .aql-page-body wrapper
         on purpose. The entrance animation applies a CSS `transform` to body children,
         which would make that element the containing block for the q-page-sticky FAB
         (position: fixed) and trap it at the end of the content flow instead of the
         viewport. Kept as a q-page child, it anchors correctly to the viewport and stays
         out of the entrance transition (a fixed FAB should not slide in). -->
    <Section
      v-if="ready && hasActionSection"
      section="PageAction"
      v-bind="pageProps"
    />
