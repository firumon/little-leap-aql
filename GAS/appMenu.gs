...
function onOpen() {
  ...
  try {
    ...
    // Non-spreadsheet context
  }

  if (activeSs && activeSs.getId() === '1POS4HDCdwHtyaIWF_qGqn51V_pL9DeTRnBva_cLk4i0') {
    menu.addSeparator()
        .addItem('GENERATE SPREADSHEET FOR NEW TENANT', 'generateSpreadsheetForNewTenant');
  }
...
function baseDialogData() {
  return {
    ...
    accessRegions: getAccessRegionsList(),
...
function handleCreateAccessRegion(form) {
  try {
    const ctx = ctxOf(CONFIG.SHEETS.ACCESS_REGIONS);
    const code = normalizeAccessRegionInputCode(form.code);
    const name = txt(form.name);
    const parent = normalizeAccessRegionInputCode(form.parent);

    if (!code) throw new Error('Code is required.');
    if (!name) throw new Error('Name is required.');
    if (findRow(ctx.sheet, ctx.idx.Code, code, 2, true) !== -1) throw new Error('Access Region code already exists.');
    if (parent && findRow(ctx.sheet, ctx.idx.Code, parent, 2, true) === -1) throw new Error('Parent Access Region not found.');
    if (parent && parent === code) throw new Error('Parent cannot be the same as Code.');
...
function handleUpdateAccessRegion(form) {
  try {
    const ctx = ctxOf(CONFIG.SHEETS.ACCESS_REGIONS);
    const originalCode = normalizeAccessRegionInputCode(form.originalCode || form.code);
    const row = findRow(ctx.sheet, ctx.idx.Code, originalCode, 2, true);
    if (row === -1) throw new Error('Access Region not found.');

    const code = normalizeAccessRegionInputCode(form.code || originalCode);
    const name = txt(form.name);
    const parent = normalizeAccessRegionInputCode(form.parent);
    ...
    const duplicate = findRow(ctx.sheet, ctx.idx.Code, code, 2, true);
    if (duplicate !== -1 && duplicate !== row) throw new Error('Access Region code already exists.');
...
function getAccessRegionDetails(code) {
  ...
  if (row === -1) return null;
  return { originalCode: get(ctx.sheet, row, ctx.idx.Code), code: get(ctx.sheet, row, ctx.idx.Code), name: get(ctx.sheet, row, ctx.idx.Name), parent: get(ctx.sheet, row, ctx.idx.Parent) };
}
...
function getResourceDetails(resourceName) {
  ...
  if (out.menu) {
    try {
      ...
      var menuGroup = Array.isArray(menuObj.group) ? menuObj.group : (Array.isArray(menuObj.groupPath) ? menuObj.groupPath : []);
      out.menuGroup = menuGroup.join(',');
...
function mapResource(form) {
  return {
    ...
    Menu: (function() {
      var editedItem = {
        group: txt(form.menuGroup).split(',').map(function(s){ return s.trim(); }).filter(Boolean),
...
function normalizeAccessRegionInputCode(code) {
  const out = (code || '').toString().trim().toUpperCase();
  if (!out) return '';
  if (!/^[A-Z]{3}[0-9]{3}$/.test(out)) {
    throw new Error('Access Region code must match AAA999 (example: UAE001).');
  }
  return out;
}
...
function generateSpreadsheetForNewTenant() {
  ...
  const response = ui.prompt(
    'Generate Spreadsheet for New Tenant',
    'Please enter the target Google Drive Folder ID:',
  ...
  if (!folderId) {
    ui.alert('Error: Folder ID cannot be empty.');
    return;
  }
  ...
  try {
    ...
    ui.alert('Error: Could not retrieve folder with ID "' + folderId + '". Please verify the ID and permissions.\n' + e.message);
    return;
  }
  ...
  try {
    ...
    appFile = templateFile.makeCopy('App', folder);
  } catch (e) {
    ui.alert('Error copying template spreadsheet (ID: ' + templateId + '):\n' + e.message);
    return;
  }
  ...
  for (let i = 0; i < fileNames.length; i++) {
    ...
    try {
      ...
      createdFiles[name] = newSs;
    } catch (e) {
      ui.alert('Error creating/copying spreadsheet "' + name + '":\n' + e.message);
      return;
    }
  }
  ...
  try {
    ...
    ui.alert('Error configuring Views/Reports Config sheet:\n' + e.message);
    return;
  }

  ui.alert(
    'Tenant spreadsheets generated successfully!\n\n' +
    'Target Folder: ' + folder.getName() + ' (' + folderId + ')\n\n' +
    'App File ID: ' + appFileId + '\n' +
    'Master File ID: ' + createdFiles['Master'].getId() + '\n' +
    'Operations File ID: ' + createdFiles['Operations'].getId() + '\n' +
    'Accounts File ID: ' + createdFiles['Accounts'].getId() + '\n' +
    'Views File ID: ' + createdFiles['Views'].getId() + '\n' +
    'Reports File ID: ' + createdFiles['Reports'].getId()
  );
}
