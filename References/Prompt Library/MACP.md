...
1. **Simple English Rule (Human-Facing Communication)**: All conversational text intended for the Human Conductor — including the Architect's discussion points, questions, design options, analysis, and the Building Agent's report-back brief — MUST be written in simple, plain English that even a low-proficiency reader or a school child reading a storybook can easily understand. Avoid long, complex, academic, or unnecessarily dense sentences.
   * **Exception — Bare Directive Prompts (AI-Facing)**: The bare Directive Prompt generated by the Architect is fed directly into the Building Agent. Therefore, Directive Prompts MAY use high-level technical terms, precise technical jargon, architecture concepts, function signatures, pseudocode, and compact, token-efficient structures as needed for the Building Agent to understand and execute cleanly.
...
9. **Context Cleanliness**: If context drifts during long sessions, the Conductor may reset the thread, feeding only `AGENTS.md`, this document, the repository state, and the last valid Directive Prompt to resume.
